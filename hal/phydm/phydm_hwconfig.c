...
#if (DM_ODM_SUPPORT_TYPE & ODM_WIN)
			if(p_dm_odm->halmac_ability & ODM_PHY_PARAM_OFFLOAD)
			{
				RT_STATUS status = RT_STATUS_SUCCESS;
				status = HAL_MAC_Config_PHY_WriteNByte(&GET_HAL_MAC_INFO(p_dm_odm->adapter),HALMAC_PARAMETER_CMD_END,0,0,FALSE,0,TRUE,0,0);
				if(status != RT_STATUS_SUCCESS)
					ret = HAL_STATUS_FAILURE;
				RT_TRACE(COMP_INIT, DBG_LOUD, ("RF param. offload status = %x\n",status));
			}
#endif				
		} else if (config_type == CONFIG_RF_TXPWR_LMT) {
...
/* JJ ADD 20161014 */
	...
	if (p_dm_odm->support_ic_type == ODM_RTL8822B) {
		if (config_type == CONFIG_BB_PHY_REG)
		{
			...
			{
				RT_STATUS status = RT_STATUS_SUCCESS;
				status = HAL_MAC_Config_PHY_WriteNByte(&GET_HAL_MAC_INFO(p_dm_odm->adapter),
														HALMAC_PARAMETER_CMD_END,
														0,
														0,
														FALSE,
														0,
				...
				if(status != RT_STATUS_SUCCESS)
					ret = HAL_STATUS_FAILURE;
				RT_TRACE(COMP_INIT, DBG_LOUD, ("BB PHY REG param. offload status = %x\n",status));									
			}
#endif	
		}
		else if (config_type == CONFIG_BB_AGC_TAB)
		{
			...
			if(p_dm_odm->halmac_ability & ODM_PHY_PARAM_OFFLOAD)
			{
				...
				if(status != RT_STATUS_SUCCESS)
					ret = HAL_STATUS_FAILURE;
				RT_TRACE(COMP_INIT, DBG_LOUD, ("BB AGC TABLE param. offload status = %x\n",status));																			
			}
#endif	
		}
		else if (config_type == CONFIG_BB_PHY_REG_PG)
...
/* JJ ADD 20161014 */
	...
	if (p_dm_odm->support_ic_type == ODM_RTL8822B)
	{
		...
		if(p_dm_odm->halmac_ability & ODM_PHY_PARAM_OFFLOAD)
		{
			...
			if(status != RT_STATUS_SUCCESS)
				ret = HAL_STATUS_FAILURE;
			RT_TRACE(COMP_INIT, DBG_LOUD, ("MAC param. offload status = %x\n",status));																			
		}
#endif	
	}
#endif
